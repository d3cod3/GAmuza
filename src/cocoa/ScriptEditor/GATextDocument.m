...
@implementation GATextDocument

-(NSArray*) syntaxDefinitionFiles
{
	if( !GASyntaxDefinitionFiles )
	{
		GASyntaxDefinitionFiles = [[NSMutableArray alloc] init];
		GASuffixToTagMappings = [[NSMutableDictionary alloc] init];
	...
	return GASyntaxDefinitionFiles;
}
...
-(void) addSyntaxFilesFromFolderToArray: (NSString*)fpath
{
	...
	while( (currName = [enny nextObject]) )
	{
		...
		[GASyntaxDefinitionFiles addObject: currPath];
	}
}
...
- (id)init
{
    ...
    if (self) {
        ...
        viewFont = [NSFont userFixedPitchFontOfSize:10];
    }
    return self;
}
...
-(BOOL)	loadDataRepresentation: (NSData*)data ofType: (NSString*)aType
{
	...
	sourceCode = [[NSString alloc] initWithData:data encoding: NSUTF8StringEncoding]; // Load the new text.
...
-(BOOL) textView:(NSTextView *)tv shouldChangeTextInRange:(NSRange)afcr replacementString:(NSString *)rps{
	...
	if( maintainIndentation )
	{
		...
		[self performSelector: @selector(didChangeText) withObject: tv afterDelay: 0.0];
	}
...
-(void)	didChangeText{
                                                     ...
                                                     || [replacementString isEqualToString:@"\r"]) )
	{
		...
		BOOL						hadSpaces = NO;
		unsigned long				lastSpace = affectedCharRange.location,
        prevLineBreak = 0;
...
-(IBAction)	recolorCompleteFile: (id)sender{
    ...
    [self recolorRange: range inView:textView];
}
...
-(void) recolorRange: (NSRange) range inView: (NSTextView *)tv{
	...
	if( tv == nil || range.length == 0	// Don't like doing useless stuff.
	...
	double diff = [[tv textStorage] length] -(range.location +range.length);
	if( diff < 0 )
		range.length += diff;
    ...
    NSMutableAttributedString*	vString = [[NSMutableAttributedString alloc] initWithString: [[[tv textStorage] string] substringWithRange: range]];
    [vString autorelease];
    ...
    [[tv textStorage] replaceCharactersInRange: range withAttributedString: vString];
              ...
              withColor: (NSColor*) col andMode:(NSString*)attr charset: (NSCharacterSet*)cset
{
    ...
    while( ![vScanner isAtEnd] )
    {
        ...
        if( vStartOffs > 0 )	// Check that we're not in the middle of an identifier:
        {
            ...
            if( [cset characterIsMember: [[s string] characterAtIndex: (vStartOffs -1)]] ){
                continue;
            }
        }
        ...
        if( (vStartOffs +[ident length] +1) < [s length] )
        {
            ...
            // If charset is NIL, this evaluates to NO.
            if([cset characterIsMember: [[s string] characterAtIndex: (vStartOffs +[ident length])]]){
                continue;
            }
        }
        ...
        if([ident isEqualToString:[[s string] substringWithRange:NSMakeRange(vStartOffs,[ident length])]]){
            //NSLog(@"%@ : %@",ident,[[s string] substringWithRange:NSMakeRange(vStartOffs,[ident length])]);
...
-(NSDictionary*)	defaultTextAttributes{
    ...
    return [NSDictionary dictionaryWithObject: viewFont forKey: NSFontAttributeName];
}
