...
//-----------------------------
void ofxControlPanel::loadSettingsClean(string xmlFile){
    ...
    for(unsigned int i = 0; i < xmlObjects.size(); i++){
        if( xmlObjects[i].guiObj != NULL ){
            ...
            for(int j = 0; j < numParams; j++){
                ...
                if(xmlObjects[i].xmlName != "EXPORT_VIDEO_FILE_DIALOG" || xmlObjects[i].xmlName != "STOP_EXPORT_VIDEO" || xmlObjects[i].xmlName != "OPEN_SCRIPT_FILE_DIALOG" || xmlObjects[i].xmlName != "SAVE_SCRIPT_FILE_DIALOG"){
                    string str = xmlObjects[i].xmlName+":val_"+ofToString(j);
                    float val = settings.getValue(str, xmlObjects[i].guiObj->value.getValueF(j));
...
void ofxControlPanel::saveSettings(){
    ...
    if(currentXmlFile == "") {
        currentXmlFile = "settings/guiSettings.xml";
    }
...
void ofxControlPanel::update(){
	...
	{
		saveButton       = ofRectangle(boundingBox.x + boundingBox.width - 100, boundingBox.y + 4, MAX(40, 8 + displayText.getTextWidth("save")) , MAX(12, displayText.getTextSingleLineHeight()) );
		restoreButton    = ofRectangle(saveButton.x + saveButton.width + 15, boundingBox.y + 4,  MAX(40, 8 + displayText.getTextWidth("load")) , MAX(12, displayText.getTextSingleLineHeight()) );
	}

	//ofxControlPanel::topSpacing = MAX(20, topBar.height);
    ofxControlPanel::topSpacing = 1;
...
void ofxControlPanel::draw(){
	...
	if ( bDoSaveRestore ){
		...
		if(bUseTTFFont) {
			guiTTFFont.drawString("SAVE", saveButton.x + 8, saveButton.y + saveButton.height - 2);
		}
		else {
			ofDrawBitmapString("SAVE", saveButton.x, saveButton.y + saveButton.height -3);
		}
        ...
        ofPushStyle();
            ...
            ofSetColor(255, 255, 255);
		if(bUseTTFFont) {
			guiTTFFont.drawString("LOAD", restoreButton.x + 8, restoreButton.y + restoreButton.height - 2);
		}
		else {
			ofDrawBitmapString("LOAD", restoreButton.x, restoreButton.y + restoreButton.height -3);
		}
		ofPopStyle();
        //////////////////////////////
	}
