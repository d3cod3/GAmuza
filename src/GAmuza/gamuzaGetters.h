...
// STRING SECTION
//--------------------------------------------------------------
void gaDrawString(string _text,int pix,float posx, float posy){
    int tempPix= 12;
    if(pix >= 1 && pix <= 140){ // 0 139 array index
        tempPix = pix;
    }
    gapp->liveCodingFont[tempPix-1].drawString(_text,posx,posy); 
}
...
/*int gaONIRunningBlob(){
	if(gapp->openniActivated && gapp->sensorKinect.useKinect){
		return gapp->sensorKinect.runningBlobs;
	}else{
		return 0;
	}
}
...
float gaONIBlobX(int _bId){
	if(gapp->openniActivated && _bId >= 0 && _bId < MAX_USERS_HARDLIMIT){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
			for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId){
					return gapp->sensorKinect._osc_blobInfo[e].center.x;
				}
			}
		}else{
			return 0.0f;
...
float gaONIBlobY(int _bId){
	if(gapp->openniActivated && _bId >= 0 && _bId < MAX_USERS_HARDLIMIT){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
			for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId){
					return gapp->sensorKinect._osc_blobInfo[e].center.y;
				}
			}
		}else{
			return 0.0f;
...
float gaONIBlobW(int _bId){
	if(gapp->openniActivated && _bId >= 0 && _bId < MAX_USERS_HARDLIMIT){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
			for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId){
					return gapp->sensorKinect._osc_blobInfo[e].size.width;
				}
			}
		}else{
			return 0.0f;
...
float gaONIBlobH(int _bId){
	if(gapp->openniActivated && _bId >= 0 && _bId < MAX_USERS_HARDLIMIT){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
			for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId){
					return gapp->sensorKinect._osc_blobInfo[e].size.height;
				}
			}
		}else{
			return 0.0f;
...
float gaONIBlobAngle(int _bId){
	if(gapp->openniActivated && _bId >= 0 && _bId < MAX_USERS_HARDLIMIT){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
			for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId){
					return gapp->sensorKinect._osc_blobInfo[e].angle;
				}
			}
		}else{
			return 0.0f;
...
int gaONIBlobContourSize(int _bId){
	if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
            if(gapp->sensorKinect.cfDetail == 0 || gapp->sensorKinect.cfDetail == 1){
                for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
                    if(gapp->sensorKinect.blobsOrder[e] == _bId){
                        return gapp->sensorKinect.contourSmooth[e].size();
                    }
                }
            }else if(gapp->sensorKinect.cfDetail == 2){
                for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
                    if(gapp->sensorKinect.blobsOrder[e] == _bId){
                        return gapp->sensorKinect.contourSimple[e].size();
                    }
                }
            }
        }else{
...
float gaONIBlobCPointX(int _bId, int _cPoint){
	if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
            if(gapp->sensorKinect.cfDetail == 0 || gapp->sensorKinect.cfDetail == 1){
                for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
                    if(gapp->sensorKinect.blobsOrder[e] == _bId && _cPoint < gapp->sensorKinect.contourSmooth[e].size()){
                        return gapp->sensorKinect.contourSmooth[e].at(_cPoint).x;
                    }
                }
            }else if(gapp->sensorKinect.cfDetail == 2){
                for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
                    if(gapp->sensorKinect.blobsOrder[e] == _bId && _cPoint < gapp->sensorKinect.contourSmooth[e].size()){
                        return gapp->sensorKinect.contourSimple[e].at(_cPoint).x;
                    }
                }
            }
        }else{
	...
	if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder){
            if(gapp->sensorKinect.cfDetail == 0 || gapp->sensorKinect.cfDetail == 1){
                for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
                    if(gapp->sensorKinect.blobsOrder[e] == _bId && _cPoint < gapp->sensorKinect.contourSmooth[e].size()){
                        return gapp->sensorKinect.contourSmooth[e].at(_cPoint).y;
                    }
                }
            }else if(gapp->sensorKinect.cfDetail == 2){
                for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
                    if(gapp->sensorKinect.blobsOrder[e] == _bId && _cPoint < gapp->sensorKinect.contourSmooth[e].size()){
                        return gapp->sensorKinect.contourSimple[e].at(_cPoint).y;
                    }
                }
            }
        }else{
...
int gaONIBlobGeometrySize(int _bId){
	if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder && gapp->sensorKinect.computeContourGeometry){
            for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId){
                    return gapp->sensorKinect.geomLines[e].size();
                }
            }
		}else{
			return 0;
...
float gaONIBlobGLineX1(int _bId, int _gLine){
    if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder && gapp->sensorKinect.computeContourGeometry){
            for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId && _gLine < gapp->sensorKinect._osc_blobGeom[e].size()){
                    return gapp->sensorKinect.geomLines[e].at(_gLine).x;
                }
            }
        }else{
			return 0.0f;
...
float gaONIBlobGLineY1(int _bId, int _gLine){
    if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder && gapp->sensorKinect.computeContourGeometry){
            for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId && _gLine < gapp->sensorKinect._osc_blobGeom[e].size()){
                    return gapp->sensorKinect.geomLines[e].at(_gLine).y;
                }
            }
        }else{
			return 0.0f;
...
float gaONIBlobGLineX2(int _bId, int _gLine){
    if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder && gapp->sensorKinect.computeContourGeometry){
            for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId && _gLine < gapp->sensorKinect._osc_blobGeom[e].size()){
                    return gapp->sensorKinect.geomLines[e].at(_gLine).z;
                }
            }
        }else{
			return 0.0f;
...
float gaONIBlobGLineY2(int _bId, int _gLine){
    if(gapp->openniActivated && _bId >= 0 && _bId < gapp->sensorKinect.runningBlobs){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder && gapp->sensorKinect.computeContourGeometry){
            for(unsigned int e = 0; e < gapp->sensorKinect.runningBlobs; e++){
				if(gapp->sensorKinect.blobsOrder[e] == _bId && _gLine < gapp->sensorKinect._osc_blobGeom[e].size()){
                    return gapp->sensorKinect.geomLines[e].at(_gLine).w;
                }
            }
        }else{
			return 0.0f;
...
float gaONIOpticalFlowX(int _index){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeOpticalFlow){
			return gapp->sensorKinect._osc_opfVel[_index].x;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIOpticalFlowY(int _index){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeOpticalFlow){
			return gapp->sensorKinect._osc_opfVel[_index].y;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIOpticalFlowVX(int _index){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeOpticalFlow){
			return gapp->sensorKinect._osc_opfVel[_index].z;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIOpticalFlowVY(int _index){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeOpticalFlow){
			return gapp->sensorKinect._osc_opfVel[_index].w;
		}else{
			return 0.0f;
		}
	}else{
...
bool gaONITrigger(int _aID){
	if(gapp->openniActivated && _aID < TRIGGER_AREAS_NUM){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.computeContourFinder && gapp->sensorKinect.computeTriggerAreas){
			return gapp->sensorKinect.triggerState[_aID];
		}else{
			return -1;
		}
	}else{
...
float gaONIHandX(int hand){
	if(gapp->openniActivated && hand < 4){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.isTrackingHands){
			return gapp->sensorKinect._osc_hands_Pos[hand].x;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIHandY(int hand){
	if(gapp->openniActivated && hand < 4){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.isTrackingHands){
			return gapp->sensorKinect._osc_hands_Pos[hand].y;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIHandZ(int hand){
	if(gapp->openniActivated && hand < 4){
		if(gapp->sensorKinect.useKinect && gapp->sensorKinect.isTrackingHands){
			return gapp->sensorKinect._osc_hands_Pos[hand].z;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIAccelerometerX(){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect){
			return gapp->sensorKinect._osc_sensorAcc.x;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIAccelerometerY(){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect){
			return gapp->sensorKinect._osc_sensorAcc.y;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONIAccelerometerZ(){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect){
			return gapp->sensorKinect._osc_sensorAcc.z;
		}else{
			return 0.0f;
		}
	}else{
...
float gaONITilt(){
	if(gapp->openniActivated){
		if(gapp->sensorKinect.useKinect){
			return gapp->sensorKinect._osc_sensorTilt;
		}else{
			return 0.0f;
		}
	}else{
